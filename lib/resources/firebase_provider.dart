...
class FirebaseProvider {
  ...
  Future<void> addPostToDb(
      User1 currentUser, String imgUrl, String caption, String location) {
  ...
  Future<User1> retrieveUserDetails(User user) async {
    DocumentSnapshot _documentSnapshot =
        await _firestore.collection("users").doc(user.uid).get();
    return User1.fromMap(_documentSnapshot.data());
  }
  ...
  Future<User1> fetchUserDetailsById(String uid) async {
  ...
  Future<List<User1>> fetchAllUsers(User user) async {
    List<User1> userList = List<User1>();
    QuerySnapshot querySnapshot =
        await _firestore.collection("users").get();
    for (var i = 0; i < querySnapshot.docs.length; i++) {
      if (querySnapshot.docs[i].id != user.uid) {
        userList.add(User1.fromMap(querySnapshot.docs[i].data()));
