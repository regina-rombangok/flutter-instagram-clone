...
class _InstaFeedScreenState extends State<InstaFeedScreen> {
  ...
  User1 currentUser, user, followingUser;
  IconData icon;
  Color color;
  List<User1> usersList = List<User1>();
  ...
  void fetchFeed() async {
    ...
    User1 user = await _repository.fetchUserDetailsById(currentUser.uid);
    setState(() {
  ...
  Widget listItem(
      {List<DocumentSnapshot> list, User1 user, User1 currentUser, int index}) {
  ...
  void postLike(DocumentReference reference, User1 currentUser) {
    var _like = Like(
  ...
  void postUnlike(DocumentReference reference, User1 currentUser) {
    reference
        .collection("likes")
        .document(currentUser.uid)
        .delete()
